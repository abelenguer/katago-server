...
pub use analysis::AnalysisStream;
...
struct PendingGuard {
    inner: Arc<Inner>,
    id: String,
    sender: mpsc::WeakUnboundedSender<Value>,
    finished: bool,
}

impl Drop for PendingGuard {
    fn drop(&mut self) {
        let removed = self.inner.pending.lock().is_ok_and(|mut map| {
            let matches = self.sender.upgrade().is_some_and(|sender| {
                map.get(&self.id)
                    .is_some_and(|pending| sender.same_channel(pending))
            });
            if matches {
                map.remove(&self.id);
            }
...
/// Keeps the pipe paired with its child during a write; the caller locks stdin.
struct WriteGuard<'a> {
    inner: &'a Inner,
    child: tokio::sync::MutexGuard<'a, Option<Child>>,
...
impl Drop for WriteGuard<'_> {
    fn drop(&mut self) {
        if self.stdin.is_some() {
            ...
            // EOF alone can leave KataGo draining long-running searches. Kill
            // this exact child now; only its stdout reader calls on_exit.
            if let Some(child) = self.child.as_mut()
                && let Err(e) = child.start_kill()
            {
                warn!("failed to kill KataGo after an incomplete stdin write: {e}");
            }
        }
    }
}
...
impl AnalysisEngine {
    ...
    pub async fn analyze(&self, query: &Query) -> Result<Vec<AnalysisResponse>> {
        ...
        let outcome = async {
            let mut stream = self.analyze_stream(query).await?;
            let mut results = Vec::with_capacity(query.expected_results());
    ...
    /// Waits for server shutdown, not for a KataGo exit or restart.
...
impl Inner {
    ...
    async fn spawn_child(self: &Arc<Self>) -> Result<()> {
        ...
        {
            // Publish both handles together, in the same lock order as writes.
    ...
    async fn probe_ready(self: Arc<Self>) {
        match self.fetch_version(READINESS_TIMEOUT).await {
            Ok(v) => {
                ...
                if stdin.is_none() || !self.alive.load(Ordering::SeqCst) {
                    return;
                }
    ...
    async fn write_line(&self, value: &Value) -> Result<()> {
        ...
        line.push(b'\n');
        timeout(ACTION_TIMEOUT, async {
            let mut guard = self.stdin.lock().await;
            let child = self.child.lock().await;
            let mut write = WriteGuard {
                inner: self,
                child,
                stdin: guard.take(),
            };
            let stdin = write.stdin.as_mut().ok_or(EngineError::ProcessDied)?;
            stdin.write_all(&line).await?;
            stdin.flush().await?;
            *guard = write.stdin.take();
            Ok(())
        })
        .await
        .map_err(|_| EngineError::Timeout(ACTION_TIMEOUT.as_secs()))?
    }
    ...
    fn register(
        self: &Arc<Self>,
        id: &str,
    ) -> Result<(PendingGuard, mpsc::UnboundedReceiver<Value>)> {
        let mut pending = self
            .pending
            .lock()
            .map_err(|_| EngineError::Parse("pending map poisoned".into()))?;
        let std::collections::hash_map::Entry::Vacant(entry) = pending.entry(id.to_owned()) else {
    ...
    /// Sends an administrative query and collects its response.
    ///
    ...
    async fn request(
        ...
        let (mut guard, mut rx) = self.register(id)?;
    ...
    async fn collect(
        ...
        loop {
            ...
            let Some(message) = classify_message(id, message)? else {
                continue;
            };
...
/// Classifies errors before deserialisation, since analysis fields have defaults.
fn classify_message(id: &str, message: Value) -> Result<Option<Value>> {
