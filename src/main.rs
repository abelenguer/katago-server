...
async fn serve_async(config: Config) -> anyhow::Result<()> {
    ...
    let ws_sessions = TaskTracker::new();
    let state = AppState {
        engine: engine.clone(),
    ...
    // HTTP graceful shutdown does not wait for upgraded WebSocket connections.
    ws_sessions.close();
    if tokio::time::timeout(Duration::from_secs(5), ws_sessions.wait())
        .await
        .is_err()
    {
        warn!("WebSocket shutdown grace period expired");
    }
    info!("bye");
    Ok(())
}
