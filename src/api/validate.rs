...
/// WebSocket requests retain a strict, nonempty set of explicitly requested turns.
/// Unlike whole-game HTTP analysis, an omitted list means only the final position.
pub fn build_ws_query(
    request: &AnalysisRequest,
    katago: &KatagoConfig,
) -> Result<PreparedQuery, ApiError> {
    if let Some(turns) = &request.analyze_turns {
        let unique: HashSet<_> = turns.iter().collect();
        if turns.is_empty() || unique.len() != turns.len() {
            return Err(ApiError::invalid_request(
                "analyzeTurns must be nonempty and must not contain duplicates",
            )
            .with_field("analyzeTurns"));
        }
    }
    build_query(request, katago, false)
}
