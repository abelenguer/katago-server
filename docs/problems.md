...
Every HTTP error response has `Content-Type: application/problem+json` and a `type`
...
Status 403. The `/api/v1/analysis/ws` upgrade supplied an `Origin` not allowed by
`server.cors_allowed_origins`; no WebSocket was established. Configure the
intended browser origin (including scheme and any port) in the allowed list.
CLI clients without `Origin` are allowed; this check is not authentication.
