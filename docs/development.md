...
When changing WS behavior, exercise the [protocol](api.md#get-apiv1analysisws)
over a real upgraded connection, not just an HTTP router call:

- Out-of-order final turns, omitted turns, strict empty/duplicate/range checks, exact client IDs and independent engine UUIDs.
